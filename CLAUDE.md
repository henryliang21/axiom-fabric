...
**MCP server (`af-mcp`) lives in the core package** and `mcp` is a **default (non-optional) dependency** — Axiom Fabric is agent-facing, so MCP ships with every install (the `[mcp]` extra is retained as a no-op for backward compatibility). It's a thin stdio adapter over the same repository functions: read tools always on (incl. `change_cost`/`list_stale`/`get_fact_source`), write tools (`create_layer`/`create_fact`/`update_fact`/`retract_fact`/`attach_source`/`refresh_fact`) gated behind `--allow-writes` (or `AF_MCP_ALLOW_WRITES=1`). `build_server(allow_writes)` in `mcp/server.py` registers tools + the `axiom_fabric_usage` prompt; `af-mcp install --client {claude|claude-desktop|gemini|codex} [--with-skill]` wires up agent configs (and, with `--with-skill`, drops the per-agent guidance file, generated from `agent_guide.md`). Guidance is single-sourced from `mcp/agent_guide.md` (canonical, served as the MCP prompt); per-agent wrapper files (Claude/Gemini/Codex) live in the workspace-root `skills/` directory.
