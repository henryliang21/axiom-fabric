...
deterministic version like `0.3.142`.

If git is not available (no `.git` directory, e.g. a source tarball without
history) we raise — by design. Builds without git are unsupported because we
cannot stamp a meaningful version. End users should install from a published
wheel, where the version was baked in at our build time; people vendoring the
source files directly into another project don't run this file at all.
"""
